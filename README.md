...
### Operating Systems:
			It is expected to work on all platforms that can successfully install all the requirements above. However, 	
			the scripts were only tested on:
...
### APPRECIATION:
		...
		My Special Appreciation therefore goes to ** Prof. David J. Malan ** (malan@harvard.eduand) and His Team of 	
		Experts, for making this course available for free to millions of persons like me. Thank You.
...
### USAGE INSTRUCTIONS:
	*** PLEASE NOTE: ***
	This is not a docking platform. It is therefore essential that where applicable, all dockings would have been done using your preferred docking software. Also, this platform does not curently provide for MDS analysis. 
	...
	***To Setup Receptor - Ligand(s) Complex:***
		...
		6). Go to Generate menu to run the selected route
			*** PLEASE NOTE: ***
			The platform provides for two approaches for generating MDS input files. They are Interactive or Noninteractive approaches. Interactive allows the users to actively interact with the system, make informed decision by taking time to answer question from input request and make changes to needed forcefields, etc. This interactive approach is recommended for new users, and will be very useful when comparing suitability of different forcefields, box types, box sizes, etc before taking final decision on default parameters to use. RLmany and RLsingle can come handy when troubleshooting with interactive mode. Experienced users or users with well defined default parameters can work with noninteractive mode. In bothe cases, other default parameters (timeout, editconf -d and -bt) should be set. The default values you see are just ok for noninteractive mode, although, you can extend the timeout, to give you amble time to study the platform. Meanwhile, Noninteractive mode will move faster by reducing the timeout.	
	...
	***To Perform Molecular Dynamic Simulation:***
		1). Click on MDS and select FRESH or CONTINUATION. FRESH will required upload of necessary files as indicated. All needed files for MDS are saved in gmxmds folder during generation of complex. The "samples" subfolder inside the working directory also contains samples of mdp files for you to edit and upload. Alternatively, if you generated your MDS input files from anoother source, please follow Manual setup approach below.
